...
L'IA accélère notre capacité à faire des prototypes, afin de pouvoir prendre les décisions produit les plus pertinentes sans engager des développements coûteux. **Le cycle de développement devient "prompt -> prototypage -> décision"** là où, avant, c'était "spécifications -> ticket -> implémentation -> démonstration".

Nicolas fait le constat, comme je peux le faire autour de moi, que l'IA aide aussi les PO/PM à explorer davantage de choses de manière plus autonome, avant de passer la main aux équipes de développement pour l'industrialisation. Il reconnaît que les aspects sécurité, conformité ou scalabilité nécessitent toujours une expertise humaine.
...
Loup Cellard (chercheur au CNRS) nous donne des exemples d'enjeux bien concrets qu'amènent les infrastructures nécessaires à l'IA (data centers, câbles sous-marins, ...). Il évoque notamment les conflits d'usages autour de l'électricité et de l'immobilier.

Il pose aussi des questions sur une souveraineté très floue sur ces sujets, majoritairement aux mains du secteur privé dont nous dépendons pour tous nos usages numériques et IA. La souveraineté étant elle-même une notion du "monde réel", difficilement applicable dans le "monde numérique" où tout est interconnecté : peut-être devrions-nous imaginer de nouveaux concepts, plus adaptés, pour nous protéger.
...
Benoît Fontaine (architecte) nous présente le nouveau "skill" (pun intended) à maîtriser : le context engineering. Mais si, vous savez... l'étape après le prompt engineering dont on parlait il y a quelques années :)

L'idée est que **le (bon) contexte est essentiel pour bénéficier pleinement de la puissance des agents IA**.

**Lorsque vous atteignez 60 % de la fenêtre de contexte de l'agent, la pertinence et la performance se dégradent fortement**. À ce moment-là, il faut "compacter" le contexte (certains outils comme Claude Code permettent de donner des indications sur les éléments les plus pertinents à conserver du contexte actuel). Si vous dépassez ce seuil, la session est foutue et vous aurez fortement intérêt à repartir d'une nouvelle session.
...
Ils ont notamment présenté des **attaques par évasion de modèle** sur des modèles qui analysent les images. Par exemple, **comment une image de char d'assaut peut être identifiée comme une ambulance** en perturbant de manière imperceptible à l'oeil nu l'image originale. Ou comment un panneau STOP peut être vu comme un panneau de limitation de vitesse. Les mêmes techniques peuvent s'appliquer aux modèles de texte : un peu de bruit dans un prompt peut rendre inefficaces les protections censées empêcher d'obtenir des informations pour fabriquer une bombe.
...
Kamila Giedrojc (lead product designer) veut nous sensibiliser aux éléments importants lors de la construction d'un dashboard, pour qu'il ne finisse pas aux oubliettes quelques jours après sa création.
